...
export function NetworkEditDetail({ serviceId, onSave, isInline = false }: NetworkEditDetailProps) {
  ...
  const handleSave = async () => {
    try {
      ...
      // Helper function to convert string numbers safely
      const parseNumberSafe = (value: string) => {
        const parsed = parseInt(value);
        return isNaN(parsed) ? undefined : parsed;
      };

      // Start with the most minimal payload possible - progressive enhancement approach
      let updateData: any = {
      ...
      // Try step 1: Minimal payload (name, ssid, enabled only)
      try {
        console.log('=== ATTEMPTING MINIMAL UPDATE ===');
        console.log('Minimal payload:', JSON.stringify(updateData, null, 2));
        
        const updatedService = await apiService.updateService(serviceId, updateData);
        ...
        toast.success('Network configuration saved successfully', {
          description: `Basic settings for ${formData.name} have been updated.`
        });
        ...
        console.log('Minimal update failed, trying with description:', minimalError);
        
        // Step 2: Add description if it exists
        if (formData.description && formData.description.trim()) {
          updateData.description = formData.description.trim();
          
          try {
            console.log('=== ATTEMPTING UPDATE WITH DESCRIPTION ===');
            console.log('Payload with description:', JSON.stringify(updateData, null, 2));
            
            const updatedService = await apiService.updateService(serviceId, updateData);
            console.log('Update with description successful!');
            ...
            console.log('Update with description failed, trying with VLAN:', descError);
          }
        }

        // Step 3: Try adding VLAN if specified
        if (formData.vlan && formData.vlan.trim()) {
          const vlanNum = parseNumberSafe(formData.vlan);
          console.log('VLAN processing:', {
            original: formData.vlan,
            parsed: vlanNum,
            isValid: vlanNum && vlanNum >= 1 && vlanNum <= 4094
          });
          
          if (vlanNum && vlanNum >= 1 && vlanNum <= 4094) {
            // Reset to minimal payload and add VLAN
            updateData = {
              name: formData.name.trim(),
              ssid: formData.ssid.trim(),
              enabled: Boolean(formData.enabled),
              dot1dPortNumber: vlanNum
            };
            
            try {
              console.log('=== ATTEMPTING UPDATE WITH VLAN ===');
              console.log('Payload with VLAN:', JSON.stringify(updateData, null, 2));
              
              const updatedService = await apiService.updateService(serviceId, updateData);
              console.log('Update with VLAN successful!');
              
              setService(updatedService);
              toast.success('Network configuration saved successfully', {
                description: `Settings including VLAN for ${formData.name} have been updated.`
              });
              ...
              console.log('Update with VLAN failed, trying original service structure:', vlanError);
            }
          }
        }

        // Step 4: Try using the original service structure with minimal changes
        ...
        delete conservativeUpdate.id; // Don't send ID in update
        delete conservativeUpdate.createdAt;
        delete conservativeUpdate.updatedAt;
        delete conservativeUpdate.createTime;
        delete conservativeUpdate.updateTime;
        
        try {
          console.log('=== ATTEMPTING CONSERVATIVE UPDATE ===');
          console.log('Conservative payload:', JSON.stringify(conservativeUpdate, null, 2));
          
          const updatedService = await apiService.updateService(serviceId, conservativeUpdate);
          console.log('Conservative update successful!');
          
          setService(updatedService);
          toast.success('Network configuration saved successfully', {
            description: `Settings for ${formData.name} have been updated using conservative approach.`
          });
          ...
          console.log('Conservative update failed, trying field-by-field approach:', conservativeError);
          
          // Step 5: Try individual field updates
          const fieldUpdateAttempts = [
            { name: formData.name.trim() },
            { ssid: formData.ssid.trim() },
            { enabled: Boolean(formData.enabled) }
          ];

          for (const fieldUpdate of fieldUpdateAttempts) {
            try {
              console.log('=== ATTEMPTING FIELD UPDATE ===');
              console.log('Field payload:', JSON.stringify(fieldUpdate, null, 2));
              
              await apiService.updateService(serviceId, fieldUpdate);
              console.log('Field update successful for:', Object.keys(fieldUpdate)[0]);
              
            } catch (fieldError) {
              console.log('Field update failed for', Object.keys(fieldUpdate)[0], ':', fieldError);
            }
          }

          // After all field attempts, try to get the updated service
          try {
            const updatedService = await apiService.getServiceById(serviceId);
            setService(updatedService);
            toast.success('Network configuration partially updated', {
              description: `Some settings for ${formData.name} may have been updated.`
            });
            ...
            console.log('Failed to retrieve updated service:', getError);
          }

          // If all approaches failed, throw the original error with enhanced information
          throw new Error(`All update approaches failed. Original error: ${minimalError.message}`);
        }
      }
      ...
      // Log specific API expectations
      console.error('API debugging hints:');
      ...
      if (errorMessage.includes('422')) {
        userFriendlyError = 'Validation failed. The server rejected the update. Check that all field values are valid and try updating fewer fields at once.';
